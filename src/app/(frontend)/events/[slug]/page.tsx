...
export default async function FullEventPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="bg-blueprint-dark overflow-hidden space-y-6 md:min-h-[calc(100vh-80px)]">
      <div className="w-full container md:pb-6 pt-6">
        <Link
          href="/events"
          className="w-fit text-3xl flex items-center font-semibold text-blueprint "
        >
          <ChevronLeft className="inline mr-2" />
          Back to Events
        </Link>
      </div>
      <div className="md:h-[700px] flex pb-10 flex-col-reverse md:flex-row container text-blueprint-50 md:space-x-6">
        <div className="md:w-1/2 w-full flex flex-col items-center md:items-start">
          <h1 className="text-5xl md:text-6xl text-center md:text-left font-bold text-blueprint">
            {event.title}
          </h1>
          ...
          <div className="md:overflow-scroll my-6">
            <RichText
              ...
              enableGutter={false}
              className=" mt-4  prose-headings:mt-2 prose-headings:mb-2 prose-headings:text-blueprint prose-p:text-blueprint-50 prose-strong:text-blueprint-50 prose-ul:text-blueprint-50 prose-ol:text-blueprint-50 prose-li:text-blueprint-50"
            />
          </div>
        </div>
        <div className="md:w-1/2 w-full flex flex-col items-center mb-6 md:mb-0">
          ...
          <div className="mt-6 w-full max-w-md">
            <h2 className="text-center text-2xl font-bold text-blueprint">
...
function DateBlock({
  ...
  return (
    <div className="flex flex-col md:flex-row items-center space-x-4">
      <div className="flex font-bold border-blueprint border-4 space-x-3 px-2 py-1 w-max rounded-xl items-center">
        <div className="text-3xl font-bold min-w-fit">{eventTime}</div>
      ...
      <div className="pt-2 flex flex-col items-center md:pt-0">
        {venue && <div className="text-3xl font-bold text-center truncate">{venue}</div>}
        {status && (
          <div className="text-2xl flex gap-2 items-center font-semibold pl-2">
            {status === 'passed' ? (
              <>
                <span className="">Passed</span>
                <span className="bg-gray-600 size-3 rounded-full " />
              </>
            ) : (
              <div>
                <span className="text-blueprint-50">
                  {status === 'upcoming' ? 'Upcoming' : 'Today'}
                </span>
                <span className="bg-green-500 size-3 rounded-full shadow-[0_0_5px_#00C950,0_0_15px_#00C950,0_0_30px_#00C950]" />
              </div>
            )}
          </div>
        )}
